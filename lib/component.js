...
(function (root, factory) {
  ...
  _.extend(Wrapper.prototype, Backbone.Events, {
    ...
    setModels: function (models, initialState, isDeferred) {
      if (typeof models !== 'undefined' && !_.isEmpty(models) &&
          (models.attributes || typeof models === 'object' &&
           _.values(models)[0].attributes)
      ) {
    ...
    setCollections: function (collections, initialState, isDeferred) {
      if (typeof collections !== 'undefined' && !_.isEmpty(collections) &&
          (collections.models ||
    ...
    startCollectionListeners: function (collection, key) {
      ...
      if (collection) {
        // Lazo noops listenTo on the server, so need to change the chained calls
        // with an explicit this.listenTo on every call
        if (collection.models) {
          this .listenTo(collection, 'add remove change sort reset',
    ...
    startModelListeners: function (model, key) {
      ...
      if (model) {
        ...
        if (model.attributes) {
          ...
          this.listenTo(model, 'error', this.onError);
          this.listenTo(model, 'request', this.onRequest);
          this.listenTo(model, 'sync', this.onSync);
          this.listenTo(model, 'invalid', this.onInvalid);
        }
